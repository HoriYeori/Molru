...
const ItemRegistration = () => {
  ...
  const addItem = async () => {
    ...
    const pubKey = getAddressFrom(privKey);
    if(!pubKey) return;
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_HOST_URL}/items`, formData);
      ...
      await NFTContract.methods
        .create(sender.address, imageUrl)
        .send({ 
          from: sender.address, 
          gas: 3000000,
          gasPrice: 10000000000, 
        });
      const tokenId = await NFTContract.methods.current().call({ from: sender.address });
      setTokenId(tokenId);
      ...
      const res2 = await axios.post(`${process.env.REACT_APP_BACKEND_HOST_URL}/items/${itemId}`, nftData);
