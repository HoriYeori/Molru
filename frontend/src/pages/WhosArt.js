...
const WhosArt = () => {
  ...
  */
  ...
  const makeNewItem = async (oldItem) => {
    ...
    for(let i=0;i<oldItem.length;i++) {
      const curItem = {
        ended: false,
        hash: oldItem[i].item_hash,
        image: await NFTContract.methods.tokenURI(oldItem[i].token_id).call(),
        onSale: false,
        title: oldItem[i].item_title,
        id: oldItem[i].token_id,
      };
      newItem[i] = curItem;
    }
  ...
  const showItems = async () => {
    ...
    try {
      ...
      setLoading(true);
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_HOST_URL}/items`);
      const itemList = res.data.data;
      setIsCollection(itemList.length !== 0);

      setItem(await makeNewItem(itemList));
    }catch(e) {
      console.error(e);
    }finally {
      setLoading(false);
  ...
  const searchItem = async (addr) => {
    ...
    try {
      setIsCollection(false);
      setLoading(true);
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_HOST_URL}/items?address=${addr}`);
      const itemList = res.data.data;
      setIsCollection(itemList.length !== 0);
