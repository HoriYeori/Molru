...
const ItemPurchase = () => {
  ...
  const getItemDetail = async () => {
    ...
    try {
      ...
      const tokenImgURI = await NFTContract.methods.tokenURI(tokenId).call();
      setUri(tokenImgURI);
      
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_HOST_URL}/items/${tokenId}`);
      const nft = res.data.data[0];
      setItem(tokenImgURI);
      setItemName(nft.item_title);
      setDescription(nft.item_description);
      setAuthor(nft.author_name);
      setAuthorAdr(nft.owner_address);
